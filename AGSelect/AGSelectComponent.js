...
requireConfig['amd']['shim']["cde/repo/components/AGComponents/AGSelect/lib/bootstrap-select"] = {
  exports: "jQuery",        
  deps: {
    "cdf/lib/jquery": "jQuery"
  }
};
requireConfig['amd']['shim']["cde/components/AGComponents/AGSelect/lib/bootstrap-select"] = {
  exports: "jQuery",        
  deps: {
    "cdf/lib/jquery": "jQuery"
  }
...
define([
	...
	'amd!./lib/bootstrap-select',
	'cdf/lib/jquery',
	'css!./AGSelect',
	'css!./lib/bootstrap-select.min'
], function(InputBaseComponent, bootstrapSelect, $) {
	...
	return InputBaseComponent.extend({
		...
		draw: function(myData) {
	    	...
	    	if (mySelf.maxWidth) {
	    		mySelect.css("max-width", mySelf.maxWidth.toString() + "px");
	    	}
			...
			if (!mySelf.value || mySelf.value != param) {
				if (mySelf.valueIfEmpty && mySelf.valueIfEmpty !== "" && !mySelf.firstTime) {
					...
					mySelf.dashboard.setParam(mySelf.parameter, param);
				}
